...
import { Film, Plus, Tv } from 'lucide-vue-next'
...
<template>
  <section class="media-search-results" aria-label="媒体搜索结果">
    ...
    <div v-else-if="items.length" class="media-search-grid">
      <article v-for="item in items" :key="`${item.media_type}-${item.id}`" :class="['media-search-card', { 'is-working': stateOf(item) && !['completed', 'failed', 'cancelled', 'partial', 'no_coverage'].includes(stateOf(item)!.status), 'is-completed': stateOf(item)?.status === 'completed' }]">
        <div class="media-search-poster"><img v-if="item.poster_path" :src="tmdbImageUrl(item.poster_path, 'w342')" :alt="titleOf(item)" loading="lazy" /><component :is="item.media_type === 'tv' ? Tv : Film" v-else :size="30" /><span>{{ item.media_type === 'tv' ? '剧集' : '电影' }}</span><div v-if="stateOf(item)" :class="['media-poster-acquisition-state', stateClass(stateOf(item))]"><b>{{ stateLabel(stateOf(item)) }}</b><i><em :style="{ width: `${Math.max(4, stateProgress(stateOf(item)))}%` }" /></i></div></div>
        <div class="media-search-copy"><div class="media-search-title-row"><h3>{{ titleOf(item) }}</h3><button type="button" class="media-acquire-button" @click="acquire(item)"><Plus :size="15" />{{ isUnreleasedMovie(item) ? '预定' : '获取' }}</button></div><p class="media-search-meta">{{ yearOf(item) || '—' }}<template v-if="item.vote_average"> · {{ item.vote_average.toFixed(1) }} 分</template></p><p class="media-search-overview">{{ item.overview || '暂无剧情简介' }}</p></div>
      </article>
    </div>
...
.media-poster-acquisition-state { position: absolute; right: 5px; bottom: 5px; left: 5px; z-index: 2; padding: 5px 6px; border-radius: 5px; color: #fff; background: rgba(12,14,18,.86); font-size: 10px; backdrop-filter: blur(5px); }.media-poster-acquisition-state b { display: block; overflow: hidden; font-weight: 650; text-overflow: ellipsis; white-space: nowrap; }.media-poster-acquisition-state i { display: block; height: 3px; margin-top: 4px; overflow: hidden; border-radius: 99px; background: rgba(255,255,255,.18); }.media-poster-acquisition-state em { display: block; height: 100%; border-radius: inherit; background: #60dd83; transition: width .25s ease; }.media-poster-acquisition-state.failed em { background: #f06c6c; }.media-poster-acquisition-state.warning em { background: #e6b45e; }
