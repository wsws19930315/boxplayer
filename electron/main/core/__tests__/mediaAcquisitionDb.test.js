...
describe('MediaAcquisitionDb', () => {
    ...
    it('deduplicates an acquired media and persists its completion notification', () => {
        const db = createDb();
        const input = {
            kind: 'movie', tmdbId: 42, mediaType: 'movie', title: '示例电影', year: 2026,
    ...
    it('completes a run without a candidate when requested episodes already exist', () => {
        const db = createDb();
        const input = {
            kind: 'patrol', tmdbId: 100, mediaType: 'tv', title: '示例剧集', year: 2026, seasonNumber: 1, missingEpisodes: [1, 2],
    ...
    it('remembers terminal dead links and skips them on future tasks', () => {
        const db = createDb();
        const input = {
            kind: 'movie', mediaType: 'movie', title: 'Dead Link Movie',
    ...
    it('allows completed media to create independent missing-season runs', () => {
        const db = createDb();
        const base = {
            tmdbId: 100, mediaType: 'tv', title: '示例剧集', year: 2026,
    ...
    it('isolates active missing-season tasks by target drive and folder', () => {
        const db = createDb();
        const base = {
            kind: 'missing', tmdbId: 301, mediaType: 'tv', title: 'Multi Drive Show', seasonNumber: 1, missingEpisodes: [2], trackingEnabled: true
        };
