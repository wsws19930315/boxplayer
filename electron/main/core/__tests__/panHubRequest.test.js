...
describe('PanHub main-process request proxy', () => {
    it('parses NDJSON events split across response chunks', async () => {
        ...
        const events = [];
        await requestPanHubStream({ url: 'https://boxplayer-api-673444103572.europe-west1.run.app/api/search/stream', method: 'POST', body: '{}' }, (event) => events.push(event), undefined, fetchImpl);
