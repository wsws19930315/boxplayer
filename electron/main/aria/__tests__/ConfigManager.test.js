...
import os from 'os';
import fs from 'fs';
import { NGOSANG_TRACKERS_BEST_CDN_URL, NGOSANG_TRACKERS_BEST_IP_CDN_URL, NGOSANG_TRACKERS_BEST_URL } from '@shared/constants';
vi.mock('electron', () => ({
    app: {
...
beforeEach(() => {
    ...
    try {
        fs.unlinkSync(path.join(tmpDir, 'system.json'));
    }
    catch { }
    try {
        fs.unlinkSync(path.join(tmpDir, 'user.json'));
    }
    catch { }
});
